...
// is what should be used for making everything 
    ...
    files.iter().for_each(|f| {
        ...
        lines.iter().for_each(|line| {
            bounds.iter().for_each(|bound| {
                let out_path =
                    PathBuf::from(format!("../../templates/sections/{}/{}/default.neoj", line, bound));
                // dbg!(&out_path);
        ...
        section_types.iter().for_each(|section_type| {
            bounds.iter().for_each(|bound| {
                ...
                let output_path = PathBuf::from(format!(
                    "../../templates/sections/{}/{}/default.neoj",
                    section_type, bound
                ));
...
fn make_category_txt_files(_bounds: Vec<&str>, _config_dir: PathBuf, files: Vec<PathBuf>) {
    files.iter().for_each(|f| {
        ...
        section_types.iter().for_each(|section_type| {
            let output_path =
                PathBuf::from(format!("../../templates/sections/{}/category.txt", section_type));
            // dbg!(&output_path);

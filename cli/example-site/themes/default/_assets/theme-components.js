class ColorSwitcher extends HTMLElement {
  ...
  setMode(mode) {
    ...
    const buttons = document.querySelectorAll(`[role="ld-mode"]`)
    buttons.forEach((button) => {
  ...
  updateAutoDisplay() {
    const els = document.querySelectorAll('[role="ld-mode"][data-mode="auto"]')
    els.forEach((el) => {
...
customElements.define(
  ...
  class SettingsGear extends HTMLElement {
    ...
    addColorModes() {
      const colorModes = document.createElement('div')
    ...
    addGear() {
      this.gear = document.createElementNS('http://www.w3.org/2000/svg', `svg`)

...
pub fn json_section<'a>(
    ...
    let (source, result) = list.iter().fold(initial_error(), |acc, item| match acc {
        Ok(v) => Ok(v),
        _ => json_full_section_finder(source, item),
    })?;
    Ok((source, result))
}

fn json_full_section_finder<'a>(
    source: &'a str,
    key: &'a str,
) -> IResult<&'a str, Section, ErrorTree<&'a str>> {
    let initial_source = source;
    let (source, _) = tag("--").context("json_full_section").parse(source)?;
    let (source, _) = space1.context("json_full_section").parse(source)?;
    let (source, r#type) = tag(key).context("json_full_section").parse(source)?;
    let (source, _) = not(tag("/")).context("json_full_section").parse(source)?;
    let (source, _) = tuple((space0, line_ending))
        .context("json_full_section")
        .parse(source)?;

...
pub fn footnote_shorthand_attr(source: &str) -> IResult<&str, SpanAttr, ErrorTree<&str>> {
    let (source, attr) = alt((
...
mod test {
    ...
    // fn footnote_shorthand_fixture(
    //     #[case] input: &str,
    //     #[case] attrs: usize,
    //     #[case] _description: &str,
    // ) {
    //     let (remainder, span) = footnote_shorthand(input).unwrap();
    //     assert_eq!(remainder, "");
    //     assert_eq!(span.attrs.len(), attrs);
    // }

    // #[rstest]
    // #[case("``\\``", "can't have a single backslash at the end of text")]
    // fn error_cases(#[case] input: &str, #[case] _desc: &str) {
    //     assert!(footnote_shorthand(input).is_err());
    // }

    // #[test]
    // fn footnote_shorthand_basic() {
    //     let source = "``ping``";
    //     let left = (
    //         "",
    //         Span {

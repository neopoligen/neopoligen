...
mod test {
    ...
    fn avoid_gt() {
        let source = "echo>>";
        let left = Ok((">>", Span::Word{text: "echo".to_string(), template: "spans/word.neojinja".to_string()}));
        let right = tag_word(source);
        assert_eq!(left, right);
    }
    ...
    fn avoid_pipe() {
        let source = "hotel|";
        let left = Ok(("|", Span::Word{text: "hotel".to_string(), template: "spans/word.neojinja".to_string()}));
        let right = tag_word(source);
        assert_eq!(left, right);
    }

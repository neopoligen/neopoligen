...
pub fn inline_standard_span_with_attributes<'a>(
    source: &'a str,
    config: &'a Config,
) -> IResult<&'a str, Span> {
...
pub fn tag_boolean_attribute(source: &str) -> IResult<&str, TagAttribute> {
    let (source, _) = tag("|")(source)?;
...
// TODO: Move this when it's solid
pub fn tag_key_value_attribute(source: &str) -> IResult<&str, TagAttribute> {
    let (source, _) = tag("|")(source)?;
    let (source, key) = is_not(":")(source)?;
    let (source, _) = tag(":")(source)?;
